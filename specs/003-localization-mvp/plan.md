...
| FR9.1-FR9.9 | Preserve snapshot recording and stored snapshot values. Display-only labels may localize but snapshot category names remain compatible. | `DashboardViewModel`, `PortfolioHistoryCoordinator`, `AssetValueSnapshot`, `NetWorthSnapshot` | Existing snapshot tests plus review that no stored category-name rewrite occurs. |
